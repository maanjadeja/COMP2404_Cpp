...
/*
* Testing functions. Some of these functions redirect cout into a string stream,
* which is converted into a string. After your program prints something, the string 
* is searched for the appropriate output, or to check that certain output is not 
* there. Afterwards cout is returned to the console. 
*/
int testResOverlap(); //TODO: finish this function by writing tests  
...
//uncomment if you complete the bonus functions
//int testUpdateReservations();


/*
* Presents a menu for the user to choose which test to run, then runs the selected test(s).
* In the event the user runs all the tests, a score is accumulated and printed.
*/
int main(){
    int choice = -1;
...
int testResOverlap(){
    ...
    Date d1(2022,9,30);
 	Date d2(2022,9,29);
    Reservation res1("Joe", d1, 11, 3);
 	Reservation res2("Bob", d2, 11, 3);

 	std::cout<<endl;
    if (!resOverlap(res1, res2, false)) score = 0;

    // TODO: add at least 4 more unique tests here
...
// There are probably nicer ways to write this using logic operators.
...
int testAddTruckOrder(){
    ...
    int found = -1;
    for (int i = 0; i < NUM_TRUCKS; ++i){
        // This returns the index of the found string, which is used
        // as the starting point of the next search.
...
int testDeleteTruck(){
    ...
    std::cout<<"deleteTruck success, 2 marks"<<endl;
    return 2;
...
int testAddResTruck(){
    ...
    score += 2;
    std::cout<<"Total score: "<<score<<" marks"<<endl;
    return score;

	...
	if res == 0:
		...
		for e in testlist:
			if (out.find(e) == -1 and err.find(e) == -1):
				print(bcolors.WARNING+e+bcolors.ENDC+" NOT FOUND")
			else:
				print(bcolors.WARNING+e+bcolors.ENDC+" FOUND, 2 marks")
				score+=2
		return score
	else: 
...
def unzip_and_test(dirname, zipfile, outof):
	...
	print(str(score)+" marks")
	
	print("\nClear function test\n")
	score += run(["valgrind","./test"], 5, '1', ["Elsa","Densel","All heap blocks were freed -- no leaks are possible"])
